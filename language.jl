...
cleaned_sentence_2 = map(lowercase, messy_sentence_2) |> x -> filter(isinalphabet, x)
...
# ╔═╡ 7dfb0d4e-1ae5-11eb-313b-f180be66c6ae
string.(alphabet)
...
# ╔═╡ c7bbd1de-1ae5-11eb-1788-d3688fa60669
sample_freqs
...
# ╔═╡ 2c1cfe70-1ae6-11eb-09d8-b538a158b291
findall(x->x==0, sample_freqs)

# ╔═╡ 92bf9fd2-f9a5-11ea-25c7-5966e44db6c6
unused_letters = let
	filter(x-> index_of_letter(x) in findall(x->x==0,sample_freqs), alphabet)
end

# ╔═╡ 471295e0-1ae7-11eb-35aa-db6d03dfb855
Plots.bar(alphabet, sample_freqs)

# ╔═╡ 37d5535e-1af1-11eb-2d9e-21f3df8c1c1e
alphabet
...
# ╔═╡ b1a5b680-1af1-11eb-3887-ff6321c81f73
sample_freq_matrix
...
double_letters = alphabet[ findall(x-> x>0, [sample_freq_matrix[i,i] for i in 1:size(sample_freq_matrix,1)])]
...
function ngrams(words, n)
	map(1:length(words)-n +1) do idx
		[words[idx+step] for step in 0:n-1]
	end
end
...
function word_counts(words::Vector)
	...
	return Dict(x => count(el -> el == x, words) for x in unique(words))
end
...
function completions_cache(grams)
	keys = unique(map(x-> x[1:end-1], grams))
	cache = Dict(key => map(x -> x[end], filter(x-> x[1:end-1]==key, grams)) for key in keys)  
...
# ╔═╡ 160890d0-1c9f-11eb-0a6b-cd13475d04d1
let
	grams = ngrams(split("to be or not to be that is the question", " "), 3)
	keys = unique(map(x-> x[1:end-1], grams))
	map(x->x[end] , keys)
# 	cache = Dict(key => map(x -> x[end], filter(x-> x==key, grams))  
end
